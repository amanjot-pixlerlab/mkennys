...
$eventId = $_POST['cityId'];
$zoneId = $_POST['zoneId'];
$state_id = $_POST['state_id'];


/*
$query = "select * from wp_schedule_event_state where statezone_id='".$zoneId."' and is_delete ='1' and state_id='".$state_id."'";
...
and wp_schedule_events.id='".$eventId."' and wp_schedule_events.id=wp_schedule_event_state.event_id and wp_schedule_events.id=adt.event_id and wp_schedule_events.status='1' and
...
$selectedZoneId = 0;
 if(count($schedules)>0){
	 foreach($schedules as $schedule){
		...
		if($today<=$sdate){
		 ...
		 if($cmpId == $schedule->schdule_eventId){
			 ...
			 if($schedule->statezone_id == $prevZone || $zoneCounter==0)
			 {
	 ...
	 $query = "SELECT start_date FROM  wp_appointment_date_time where event_id =".$eventId;	
  ...
  $query="select * from $table_appointment_date_time where event_id='".$eventId."' and start_date='".$scheduleDate['0']."'";	
 ...
 // print_r($appointment_date_time_rec);
  ...
  $appointment_timesData='';
   ...
   for($i=$start;$i<=$end;$i+=1800){ 
	...
	echo json_encode(array('status'=>true, 'dates'=>$datess,'times'=>$timeData));
}else{
	echo json_encode(array('status'=>true, 'dates'=>$datess,'times'=>$timeData));
}
exit;
