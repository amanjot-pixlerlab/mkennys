...
add_submenu_page( 'mkenny-admin-page.php', 'Schedule', 'Schedule', 'manage_options', 'mkenny-admin-schedule.php', 'mkenny_admin_schedule_page'); 
...
add_submenu_page( 'mkenny-admin-page.php', 'Mailing List', 'Mailing List', 'manage_options', 'mkenny-admin-mailing-list.php', 'mkenny_admin_mailing_list_page'); 
...
//$today = date("Y-m-d");	
//$upcomingdate = date("Y-m-d", strtotime($retrievedata->start_date));	
		...
		$eventMonth_upcoming = $startDate['0'];
		// for local
		//$upevent .= '<a href="/mkennys/tour-schedule/?action='.$retrievedata->event_id.'">';
		
		//for live
		
		// $upevent .= '<a href="/tour-schedule/?action='.$retrievedata->event_id.'">';
		$upevent .= '<a href="'.home_url().'/new-tour-schedule/#'.$statesCombine.'">';
		
		$upevent .= "<b>".$eventMonth_upcoming.' '.$eventDate_upcoming."</b>";
		...
		$upevent .= "<p><b>".$eventMonth_upcoming.' '.$eventDate_upcoming."</b></p>";
...
function my_action(){
	...
	if(count($retrieve_data)==0){
		...
		$option .='<th scope="row"><label for="state_zone">State Zone</label></th>';
				$option .='<td>';
					$option .= '<select name="state_zone" class="regular-text" id="state_zone" class="state_zone">';		    foreach ($retrieve_data as $tour_scheduling){
								$selected='';
